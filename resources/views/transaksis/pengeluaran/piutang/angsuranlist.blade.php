            ...
            <div class="box box-success">
              ...
              <!-- form start -->
                <div class="row">
                  <div class="col-md-12">
                    <div class="box-body">
                    <form action="{{route('listangsuranpengeluaran')}}" method="post">
                ...
                <div class="box-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered">
                            ...
                            <tr id="{{$data->id}}">
                                <td><a href="/transaksi/pengeluaran/angsuran/report/detail/{{encrypt($data->id)}}" target="_blank">#{{$data->id}}</a></td>
                                <td>{{$data->namapenerima}}</td>
                                <td>{{$data->hppenerima}}</td>
                                <td style="width: 150px;min-width:90px;" >{{date('d-m-Y',strtotime($data->tanggal_angsuran))}} {{date("H:i:s",strtotime($data->created_at))}}</td>
                                ...
                                @if ($data->sisa_angsuran!=0)
                                    ...
                                    Rp. {{number_format(floatval($data->sisa_angsuran),2,',','.')}}
                                    </span></td>
                                @else
                                    <td id="sisa{{$data->id}}" style="width: 150px;min-width:140px;">Rp. {{number_format(floatval($data->sisa_angsuran),2,',','.')}}</td>                 
                                @endif
                                <td style="width: 150px;min-width:140px;">Rp. {{number_format(floatval($data->total_pengeluaran),2,',','.')}}</td>
                                <td>{{$data->metode_pembayaran}}</td>
                                <td><a href="/transaksi/pengeluaran/angsuran/report/{{encrypt($data->idtrans)}}" target="_blank">#{{$data->idtrans}}</a></td>
                                <td>{{$data->Nama_Cabang}}</td> 
                                <td>{{$data->username}}</td>    
                                <td style="width: 150px;min-width:50px;">
                                    <div class="btn-group">
                                        <button type="button" class="printbutton2 btn btn-danger btn-xs" data-id="{{encrypt($data->id)}}"><i class="fa fa-print"></i></button>                                        
                                    </div>
                                </td>                                                           
                            </tr>
                            @endforeach
